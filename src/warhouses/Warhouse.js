...
class Warhouse extends Component {
  ...
  async handleDelete(e) {
    ...
    if (result) {
      ...
      const response = await fetch(
        "http://localhost:3000/warehouses?id=" + newId,
        requestOptions
      );
      const data = await response.json();
  ...
  handleUpdateClick(e) {
    let id = e.target.id;
    const filtered = this.state.warhouses.filter(
      (warhouse) => warhouse._id == id
    );
